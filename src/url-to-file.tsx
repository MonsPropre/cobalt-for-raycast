...
function getCobaltError(error: CobaltError) {
    ...
    if (error.context) {
        errorMessage = Object.keys(error.context).reduce(
            (acc, key) => {
...
function fixProtocol(url: string, protocol: string) {
    ...
    export default function Command() {
        ...
        // Ajout unique de la custom instance (non dupliquée !)
        ...
        return (
            ...
            >
                <Form.Dropdown title="Instance" {...itemProps.instance}>
                    {enableCustomInstance && cobaltInstanceUrl && (
                    ...
                    <Form.Dropdown.Section title="Public Instances">
                        {instances
                            .filter(
                                ...
                                // && (instance.services === undefined || Object.values(instance.services).some((service) => typeof service === "string" ? service === "true" : service === true))
                            )
                            .sort(
                                (a, b) =>
                                    (b.score ?? 0) - (a.score ?? 0),
                    ...
                    {/* Optionnel : cas vide si aucune instance */}
                    {!instances.length && (
